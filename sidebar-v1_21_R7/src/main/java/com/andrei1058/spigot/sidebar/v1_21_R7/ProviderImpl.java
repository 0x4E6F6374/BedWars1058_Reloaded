...
public class ProviderImpl extends SidebarProvider {
    ...
    public SidebarObjective createObjective(@NotNull WrappedSidebar sidebar, String name, boolean health, SidebarLine title, int type) {
        return ((SidebarImpl)sidebar).createObjective(name, health ? getCriteria("health") : getCriteria("dummy"), title, type);
    }
    ...
    private static IScoreboardCriteria getCriteria(String name) {
        return IScoreboardCriteria.a(name).orElseThrow();
    }
