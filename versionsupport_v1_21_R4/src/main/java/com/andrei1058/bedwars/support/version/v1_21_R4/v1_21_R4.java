package com.andrei1058.bedwars.support.version.v1_21_R4;
...
import com.andrei1058.bedwars.support.version.v1_21_R4.despawnable.DespawnableAttributes;
import com.andrei1058.bedwars.support.version.v1_21_R4.despawnable.DespawnableFactory;
import com.andrei1058.bedwars.support.version.v1_21_R4.despawnable.DespawnableType;
...
public class v1_21_R4 extends VersionSupport {

    private final DespawnableFactory despawnableFactory;

    public v1_21_R4(Plugin plugin, String name) {
        super(plugin, name);
        loadDefaultEffects();
    ...
    public String getTag(org.bukkit.inventory.ItemStack itemStack, String key) {
        ...
        return meta.getPersistentDataContainer().get(new NamespacedKey("bedwars1058", key), PersistentDataType.STRING);
    }
    ...
    public boolean isProjectile(org.bukkit.inventory.ItemStack itemStack) {
        if (itemStack == null) return false;
        String name = itemStack.getType().name();
        return name.equals("SNOWBALL") || name.equals("EGG") || name.equals("ENDER_PEARL")
                || name.equals("ENDER_PEARL") || name.equals("FIRE_CHARGE") || name.equals("TRIDENT")
                || name.equals("ARROW") || name.equals("SPECTRAL_ARROW") || name.equals("EXPERIENCE_BOTTLE")
                || name.endsWith("SPLASH_POTION") || name.endsWith("LINGERING_POTION");
    }
    ...
    public double getDamage(org.bukkit.inventory.ItemStack i) {
        if (i == null) return 0;
        double[] damage = {0};
        getNmsItemCopy(i).a(EnumItemSlot.a, (attr, modifier) -> {
            if (attr.a(MinecraftKey.a("attack_damage")) && modifier.c() == AttributeModifier.Operation.a) {
                damage[0] += modifier.b();
            }
        });
        return damage[0];
    ...
    public void voidKill(Player p) {
        ...
        player.a(player.dW().n(), 1000);
    }

    @Override
    public void hideArmor(@NotNull Player victim, Player receiver) {
        List<Pair<EnumItemSlot, ItemStack>> items = new ArrayList<>();
        items.add(new Pair<>(EnumItemSlot.f, ItemStack.l));
        items.add(new Pair<>(EnumItemSlot.e, ItemStack.l));
        items.add(new Pair<>(EnumItemSlot.d, ItemStack.l));
        items.add(new Pair<>(EnumItemSlot.c, ItemStack.l));
    ...
    public void registerTntWhitelist(float endStoneBlast, float glassBlast) {
        try {
            ...
            for (Material mat : Material.values()) {
                if (!mat.name().endsWith("STAINED_GLASS")) continue;
                try {
                    field.set(CraftMagicNumbers.getBlock(mat), glassBlast);
                } catch (Exception ignored) {
                }
            }
            field.set(CraftMagicNumbers.getBlock(Material.TINTED_GLASS), glassBlast);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public org.bukkit.inventory.ItemStack setTag(org.bukkit.inventory.ItemStack itemStack, String key, String value) {
        ...
        ItemMeta meta = itemStack.getItemMeta();
        if (meta != null) {
            meta.getPersistentDataContainer().set(new NamespacedKey("bedwars1058", key), PersistentDataType.STRING, value);
    ...
    public boolean isCustomBedWarsItem(org.bukkit.inventory.ItemStack i) {
        return getTag(i, VersionSupport.PLUGIN_TAG_GENERIC_KEY) != null;
    }

    @Override
    public String getCustomData(org.bukkit.inventory.ItemStack i) {
        return getTag(i, VersionSupport.PLUGIN_TAG_GENERIC_KEY);
    }
    ...
    public String getShopUpgradeIdentifier(org.bukkit.inventory.ItemStack itemStack) {
        String tier = getTag(itemStack, VersionSupport.PLUGIN_TAG_TIER_KEY);
    ...
    public org.bukkit.inventory.ItemStack getPlayerHead(Player player, org.bukkit.inventory.ItemStack copyTagFrom) {
        ...
        if (copyTagFrom != null) {
            ...
            if (fromMeta != null && headMeta != null) {
                String bwTag = fromMeta.getPersistentDataContainer().get(
                        new NamespacedKey("bedwars1058", VersionSupport.PLUGIN_TAG_GENERIC_KEY), PersistentDataType.STRING);
                if (bwTag != null) {
                    headMeta.getPersistentDataContainer().set(
                            new NamespacedKey("bedwars1058", VersionSupport.PLUGIN_TAG_GENERIC_KEY), PersistentDataType.STRING, bwTag);
                }
                String tier = fromMeta.getPersistentDataContainer().get(
                        new NamespacedKey("bedwars1058", VersionSupport.PLUGIN_TAG_TIER_KEY), PersistentDataType.STRING);
                if (tier != null) {
                    headMeta.getPersistentDataContainer().set(
                            new NamespacedKey("bedwars1058", VersionSupport.PLUGIN_TAG_TIER_KEY), PersistentDataType.STRING, tier);
                }
            }
    ...
    public void sendPlayerSpawnPackets(Player respawned, IArena arena) {
        ...
        Location location = respawned.getLocation();
        PacketPlayOutSpawnEntity show = new PacketPlayOutSpawnEntity(
                respawned.getEntityId(),
                respawned.getUniqueId(),
                location.getX(),
                location.getY(),
                location.getZ(),
                location.getYaw(),
        ...
        PacketPlayOutEntityHeadRotation head = new PacketPlayOutEntityHeadRotation(entityPlayer, getCompressedAngle(location.getYaw()));
            ...
            if (p.getWorld().equals(respawned.getWorld())) {
                if (respawned.getLocation().distance(p.getLocation()) <= arena.getRenderDistance()) {
                    ...
                    if (p.hasPotionEffect(PotionEffectType.INVISIBILITY)) {
                        ...
                        Location boundLoc = p.getLocation();
                        PacketPlayOutSpawnEntity show2 = new PacketPlayOutSpawnEntity(
                                p.getEntityId(),
                                p.getUniqueId(),
                                boundLoc.getX(),
                                boundLoc.getY(),
                        ...
                        PacketPlayOutEntityHeadRotation head2 = new PacketPlayOutEntityHeadRotation(boundTo, getCompressedAngle(boundLoc.getYaw()));
            ...
            if (spectator.getWorld().equals(respawned.getWorld())) {
                if (respawned.getLocation().distance(spectator.getLocation()) <= arena.getRenderDistance()) {
                    ...
                    this.sendPackets(
                            ...
                            new PacketPlayOutEntityEquipment(respawned.getEntityId(), list),
                            new PacketPlayOutEntityHeadRotation(entityPlayer, getCompressedAngle(location.getYaw()))
                    );
                }
            }
        }
    ...
    public Fireball setFireballDirection(Fireball fireball, @NotNull Vector vector) {
        fireball.setVelocity(vector.clone().multiply(0.1D));
        return fireball;
    }
    ...
    public void playRedStoneDot(@NotNull Player player) {
        ...
        PacketPlayOutWorldParticles particlePacket = new PacketPlayOutWorldParticles(
                new ParticleParamRedstone(
                        (color.getRed() << 16) | (color.getGreen() << 8) | color.getBlue(), (float) 1
),
                true,
                false,
    ...
    private void sendPackets(Player player, Packet<?> @NotNull ... packets) {
        net.minecraft.server.network.PlayerConnection connection = ((CraftPlayer) player).getHandle().f;
